...
const SECURITY = {
  ...
  camera:   'Camera check-in is optional and off unless you turn it on. The camera only runs while you hold a check-in open, never in the background. The video stays on your device — only a few numbers describing how much movement was observed are sent, and the video itself is discarded the moment the check-in ends.',
  demo:     'This is a hackathon prototype. Please use fictional information only.'
};

/* Derived camera numbers only. Anything that could reconstruct an image is
   dropped here, so no raw frame can reach the database even by mistake. */
function cleanCamera(cam){
  if (!cam || typeof cam !== 'object') return null;
  const num = (v, lo, hi) => { const n = Number(v); return isFinite(n) ? Math.max(lo, Math.min(hi, n)) : 0; };
  if (cam.ok === false)
    return { ok:false, reason: ['denied','unsupported','nocamera','error','stopped'].indexOf(cam.reason) >= 0 ? cam.reason : 'error' };
  return {
    ok:      true,
    change:  num(cam.change, 0, 100),
    still:   num(cam.still, 0, 1),
    frames:  num(cam.frames, 0, 100000),
    seconds: num(cam.seconds, 0, 600),
    quality: cam.quality === 'poor' ? 'poor' : 'good',
    face:    !!cam.face
  };
}

/* The actions a caseworker can take on a flagged case. Each one records
   who did it and when, so the pipeline from alert to outcome is auditable. */
const ACTIONS = {
  contact: { label:'Contacted survivor', contact:true,
             text:'Spoke with the survivor and confirmed immediate safety.' },
  counsel: { label:'Referred to counsellor', contact:true, closes:'counsel',
             text:'Referred to a counsellor. First session being arranged.' },
  legal:   { label:'Referred to legal advocate', contact:true, closes:'legal',
             text:'Referred to a legal advocate for case guidance and representation.' },
  safety:  { label:'Safety concern reviewed', contact:true,
             text:'Safety concern reviewed with the survivor. Risks documented and a safety plan agreed.' }
};
...
async function handle(req, res, pathname){
  ...
  if (route[0] === 'profile' && req.method === 'PATCH'){
    ...
    if (body.consents){
      const had = !!(c.consents && c.consents.camera);
      c.consents = body.consents;
      const now_ = !!c.consents.camera;
      if (had !== now_){
        /* consent changes belong in the log the survivor can read */
        logAccess(c, me, now_ ? 'turned camera check-in ON' : 'turned camera check-in OFF');
        if (!now_){
          /* withdrawal removes the signals already collected */
  ...
  if (route[0] === 'checkin' && req.method === 'POST'){
    ...
    /* the camera is carried alongside the sitting and never enters scoreSitting */
    const cam = (c.consents && c.consents.camera) ? cleanCamera(body.camera) : null;
  ...
  if (route[0] === 'case' && route[1]){
    ...
    if (req.method === 'POST' && route[2] === 'action'){
      ...
      const follow = /^\d{4}-\d{2}-\d{2}$/.test(body.followUp || '') ? body.followUp : '';
      /* every action a worker takes is stamped with who, when and what */
      const stamp  = extra => { c.actions = c.actions || [];
        c.actions.unshift(Object.assign({ t: Date.now(), kind, by: me.name, note, followUp: follow }, extra || {}));
        c.actions = c.actions.slice(0, 80); };

      if (kind === 'review'){
        c.events.push(Engine.ev('review', Date.now(), { text:`Case reviewed by ${me.name}. Human-review gate cleared — the alert was seen and acted on by a named person.${note ? ' Note: ' + note : ''}` }));
        c.reviewedAt = Date.now(); stamp();
      } else if (kind === 'followup'){
        c.followUp = follow || '';
        c.events.push(Engine.ev('review', Date.now(), { text: follow
          ? `Follow-up scheduled for ${follow} by ${me.name}. If it does not happen, this case re-surfaces automatically.`
          : 'Follow-up scheduled. If it does not happen, this case re-surfaces automatically.' }));
        stamp();
      } else if (kind === 'resolve'){
        c.resolved = true;
        c.events.push(Engine.ev('review', Date.now(), { text:`Case closed by ${me.name}.${note ? ' ' + note : ''} It re-opens automatically if new indicators appear.` }));
        stamp();
      } else if (ACTIONS[kind]){
        const def  = ACTIONS[kind];
        const text = (note || def.text) + ` (${def.label} by ${me.name})`;
        c.interventions.push({ t: Date.now(), note: note || def.text, closes: body.closes || def.closes || '', by: me.name, kind });
        c.events.push(Engine.ev('intervention', Date.now(), { text, closes: body.closes || def.closes || '' }));
        if (def.contact) c.lastContact = Date.now();
        if (follow) c.followUp = follow;
        stamp();
      } else if (kind === 'intervention'){
        if (!note) return fail(res, 400, 'Describe what was actually done.');
        c.interventions.push({ t: Date.now(), note, closes: body.closes || '', by: me.name, kind:'intervention' });
